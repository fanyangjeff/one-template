...
public:
    /*
     public functions
     */
  //  Hash( int (*cmp)(const T*, const K&), long (*hash)(const K&, const int), int SIZE, int size_bucket);
    Hash(int (*cmp)(const T, const T), long (*hash)(const T, const int), int SIZE, int size_bucket);
 ...
 */
template<class T>
Hash<T>::Hash(int (*cmp)(const T, const T), long (*hash)(const T, const int),int SIZE, int size_bucket)
{
 ...
 */
template<class T>
bool Hash<T>::findEntry(T &dataOut)
{
 ...
 */
template<class T>
bool Hash<T>::_insertion(T &data, const int address)
{
 ...
 */
template<class T>
bool Hash<T>::_remove(const long address, T &dataOut)
{
    ...
    for(int i=0; i<size_bucket; i++)
    {
        if(bucket[address].empty[i] == false)
        {
         ...
         //   std::cout<<dataOut<<std::endl;       //debug
            ...
            if(cmp(*bucket[address].data[i], dataOut) == 1)
            {
                //  cout<<key<<endl;       //debug
                cout<<"good"<<endl;
...
template<class T>
bool Hash<T>::_find(const long address, T &dataOut)
{
